...
class TestOxfordProcessor(unittest.TestCase):
    ...
    def test_oxford_processor(self):
        ...
        df.info()
        df = oxford_processor.process(oxford_loader.load(datasets_constants.PATH_DATA_HISTORICAL))
        df.info()
        df = oxford_processor.process(oxford_loader.load(datasets_constants.PATH_DATA_FUTURE))
        df.info()
